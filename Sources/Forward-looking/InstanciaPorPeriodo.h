...
struct InstanciaPorPeriodo{
    ...
    int m; //Número de objetos de entrada
    int mHR;
    int m_bar; //Número de objetos de entrada mais número de sobras do período anterior (m[p]+2*m[p-1]+1)
    int d; //Quantidade de itens do catálogo
    int p; //Produtos distintos do período
    int periodo; //Periodo dos dados
    vetor(int) w_bar; //Comprimento dos itens do catálogo (itens do catálogo + itens do proximo período)
    vetor(int) h_bar; //Altura dos itens do catálogo (itens do catálogo + itens do proximo período)
    vetor(int) n_q;
    vetor(int) o_q;
    vetor(Item) item; //Itens do período
    vetor(Objeto) objeto; //Objetos do perído
    vetor(Objeto) getObjetosValidos(){
        unsigned long objValidos = 0;
        for (int j = 1; j <= m_bar; j++)
            if (objeto[j].ehObjetoValido()) objValidos++;
        vetor(Objeto) obj(objValidos);
        for(int j=1, j1=1; j<=m_bar; j++)
    ...
    int adicionaObjetoSobraSuperior(Objeto& sobra){
        ASSERT(sobra.codigo.periodo + 1 == periodo);
        int idx = ((sobra.codigo.objeto)*2-1)+m+1;
        if (!(idx <= m_bar)) {
