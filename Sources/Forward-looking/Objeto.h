...
public:
    ...
    double x, y;
    Par id;
    tipoCusto c_bar; //Custo por unidade de área da sobra
    bool ehSobra;
    ...
    Objeto(int _w=0, int _h=0, tipoCusto _c=0, double _x=0, double _y=0, Par _id={0,0}, tipoCusto _c_bar=0, bool sobra=false, Par cod={0,0}, Par codObjOrigem={0,0}):
    ...
    inline tipoCusto getCusto() const {
        return w*h*c;
    }
    inline tipoCusto getCustoPorUnidadeArea() const {
        return c;
    }
    inline tipoCusto getCustoSobra() const {
        return w*h*c_bar;
    }
    inline tipoCusto getCustoSobraPorUnidadeArea() const {
        return c_bar;
    }
    ...
    inline tipoCusto getValorSobraRemanescente(){
...
#endif
            delta.sobraDireita = (1.0 - (SIGMA))*delta.sobraDireita + (SIGMA)*tx;
            //delta.sobraDireita = delta.sobraDireita + (tx - delta.sobraDireita)*(SIGMA);//pow(valorSigma, iteracao);
            deltaMudou = true;
        }
        else
            deltaMudou = false;
...
#endif
            delta.sobraSuperior = (1.0 - (SIGMA))*delta.sobraSuperior + (SIGMA)*tx;
            //delta.sobraSuperior = delta.sobraSuperior + (tx - delta.sobraSuperior)*(SIGMA);//pow(valorSigma, iteracao);
            deltaMudou = true;
        }
        else
            deltaMudou = false;
...
#endif
            delta.sobras = delta.sobras + (tx - delta.sobras)*(SIGMA);//pow(valorSigma, iteracao);
            deltaMudou = true;
        }
        else
            deltaMudou = false;
...
#endif
            delta.objeto = delta.objeto + (tx - delta.objeto)*(SIGMA);//pow(valorSigma, iteracao);
            deltaMudou = true;
        }
        else
            deltaMudou = false;
